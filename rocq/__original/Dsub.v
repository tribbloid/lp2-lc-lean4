...
Proof.
  ...
  pick_fresh x. eapply ((proj1 open_rec_lc_core) T2 0 (trm_fvar x)); eauto.
  pick_fresh x. eapply ((proj2 open_rec_lc_core) e1 0 (trm_fvar x)); eauto.
Qed.
...
Proof.
  apply wf_mutind; intros; subst; eauto;
    try solve [apply_fresh wft_all as Y; eauto; apply_ih_bind H0; eauto];
    try solve [destruct (binds_middle_inv b) as [K|[K|K]];
               [ apply (@wfe_var U); eapply binds_concat_right; eauto
               | destruct K as [Fr [Eq1 Eq2]]; subst; apply (@wfe_var U0); apply binds_middle_eq; auto  
               | apply (@wfe_var U); eapply binds_concat_left; eauto ]];
    try solve [apply_fresh wfe_abs as y; eauto; apply_ih_bind H0; eauto].
Qed.
...
Proof.
  ...
  - (* wft_sel *)
    destruct o as [? | [? ?]].
    + apply wft_sel; eauto. left. apply subst_e_value. assumption. eapply wfe_term; eauto.
    + subst. simpl. case_var; eauto.
      * apply_empty wft_weaken; eauto.
      * apply wft_sel; eauto. rewrite <- ((proj2 subst_fresh) (trm_fvar x) Z u); eauto.
        simpl. auto.
  - (* wft_all *)
  ...
  - (* wfe_var *)
    ...
    + destruct (binds_concat_inv b) as [?|[? ?]].
      ...
      * destruct (binds_push_inv H3) as [[? ?]|[? ?]].
        ** subst. false~.
        ** apply wfe_var. eapply binds_concat_left; eauto.
  - (* wfe_abs *)
...
Proof.
  ...
  rewrite concat_empty_r in *.
   lets (?&?&?): (okt_push_inv O). apply okt_push; eauto.
  rewrite concat_assoc in *.
  lets (?&?&?): (okt_push_inv O).
  apply okt_push. apply IHF; eauto. eapply wft_narrow; eauto. auto.
Qed.
...
Proof.
 introv O V W. induction F using env_ind.
  rewrite map_empty. rewrite concat_empty_r in *.
   lets: (okt_push_inv O). destruct H as [? [? ?]]. eauto.
  rewrite map_push. rewrite concat_assoc in *.
   lets (?&?&?): (okt_push_inv O).
   apply okt_push. eapply IHF; eauto. eapply wft_subst; eauto. eauto.
Qed.
...
Proof.
  ...
  rewrite notin_singleton. intro. subst. apply (binds_fresh_inv b H).
...
split; [| split; eauto]. destruct H as [? [? A]].
  split; [| split; eauto]. destruct H as [? [? A]]. inversion A; subst. assumption.
  split. eauto. split;
   apply_fresh wft_all as Y; eauto;
    forwards~: (H0 Y); apply_empty (@wft_narrow T1); eauto.
split; [| split; eauto]. apply wft_sel. left. apply value_mem. eapply wfe_term; eauto. eapply wfe_mem; eauto.
  split; [| split; eauto]. apply wft_sel. left. apply value_abs. eapply wfe_term; eauto. assumption.
Qed.
...
split; [| split; eauto].
  split. eauto. split.
   pick_fresh y. specializes H0 y. destruct H0.
    forwards: okt_push_inv; eauto.
   apply_fresh wfe_abs as y; eauto.
     pick_fresh y. forwards~ K: (H0 y). destruct K as [? ?].
       forwards: okt_push_inv; eauto.
     forwards~ K: (H0 y). destruct K as [? [? ?]]. auto.
   apply_fresh wft_all as Y; eauto.
     pick_fresh y. forwards~ K: (H0 y). destruct K as [? [? ?]].
      forwards: okt_push_inv; eauto.
     forwards~ K: (H0 Y). destruct K as [? [? ?]].
      forwards: okt_push_inv; eauto.
split; [| split; eauto].
  split; [| split; eauto].
  split; [| split; eauto].
  split; [| split; eauto]. destruct (sub_regular H0) as [? [? ?]].
Qed.
...
Lemma sub_reflexivity : forall E T,
  ...
  sub E T T .
Proof.
  introv Ok WI. lets W: (wft_type WI); eauto. gen E.
  induction W; intros; inversions WI; eauto.
  destruct b. eapply sub_mem_true; eauto. eapply sub_mem_false; eauto.
  ...
  - case_var.
    ...
    + destruct (binds_concat_inv b) as [?|[? ?]].
      * eapply has_var. eauto.
        apply binds_concat_right. apply binds_map. eassumption.
      * apply has_var. eapply okt_subst; eauto.
...
Proof.
  ...
  - assumption.
  - subst. inversion Hwf; subst. false. eapply binds_empty_inv; eauto.
Qed.
...
Proof.
  ...
  - inversion Hpt; subst.
    ...
    * apply pt_mem_false. eapply psub_trans; eauto.
  - inversion Hpt; subst. eapply pt_mem_true; eauto.
...
Proof.
  ...
  induction W; intros; inversions WI; eauto.
  destruct b. eapply psub_mem_true; eauto. eapply psub_mem_false; eauto.
...
Proof.
  ...
  - specialize (H eq_refl).
    ...
    apply psub_sub in H3. eauto.
    assumption.
  - specialize (H eq_refl).
    inversion H; subst.
    eapply psub_trans. eassumption. eapply psub_sel2.
    apply psub_sub in H1. eauto.
  - apply_fresh psub_all as y; eauto.
...
Proof.
  ...
  - apply pt_mem_true. eapply psub_reflexivity; eauto. eapply psub_reflexivity; eauto.
