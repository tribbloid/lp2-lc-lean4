...
-- Coq: Lp2lc_coq\Active\Ddia.v:25-41
inductive typ : Type :=
  | top : typ
  | arr : typ → typ → typ
  | all : trm → typ → typ
...
-- Coq: Lp2lc_coq\Active\Ddia.v:43-63
...
-- Coq: Lp2lc_coq\Active\Ddia.v:181-187
inductive okt : env → Prop :=
  | okt_empty : okt []
  | okt_push : ∀ E x T, okt E → (x ∉ E.keys) → wft E T → okt ((x, T) :: E)

-- Coq: Lp2lc_coq\Active\Ddia.v:189-262
...
-- Coq: Lp2lc_coq\Active\Ddia.v:264-294
...
-- Coq: Lp2lc_coq\Active\Ddia.v:296-310
...
-- Coq: Lp2lc_coq\Active\Ddia.v:312-317
def preservation := ∀ e e' T, typing [] e T → red e e' → typing [] e' T

-- Coq: Lp2lc_coq\Active\Ddia.v:317-317
def progress := ∀ e T, typing [] e T → value e ∨ (∃ e', red e e')

-- Coq: Lp2lc_coq\Active\Ddia.v:331-353
...
-- Coq: Lp2lc_coq\Active\Ddia.v:355-373
