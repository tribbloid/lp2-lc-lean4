...
-/
def Eval: (e: Closed Trm) -> (fuel : Nat) → Option (Closed Val) :=
  let value : Closed Trm -> Closed Val := fun e Index =>
    match e Index with
    | .literal code t => ⟨.literal code t, by trivial⟩
    | .monoFn body tIn tOut => ⟨.monoFn body tIn tOut, by trivial⟩
    | _ => ⟨.literal "" .base, by trivial⟩
  let function : Closed Trm -> Closed Trm := fun e Index =>
    match e Index with
    | .monoApply function _ => function
    | _ => .literal "" .base
  let argument : Closed Trm -> Closed Trm := fun e Index =>
    match e Index with
    | .monoApply _ argument => argument
    | _ => .literal "" .base
  let rec substitute {Index : Type} (argument : Closed Val) : Trm (Option Index) -> Trm Index
    | .var none _ => (argument Index).1
    | .var (some name) t => .var name t
    | .literal code t => .literal code t
    | .monoFn body tIn tOut => .monoFn (fun name => substitute argument (body (some name))) tIn tOut
    | .monoApply function argumentTrm => .monoApply (substitute argument function) (substitute argument argumentTrm)
  let body : Closed Val -> Closed Val -> Closed Trm := fun function argument Index =>
    match function (Option Index) with
    | ⟨.monoFn functionBody _ _, _⟩ => substitute argument (functionBody none)
    | _ => .literal "" .base
  let rec eval : Closed Trm -> Nat -> Option (Closed Val)
    | _, 0 => none
    | e, fuel + 1 =>
        match e PUnit with
        | .literal _ _ | .monoFn _ _ _ => some (value e)
        | .var _ _ => none
        | .monoApply _ _ =>
            match eval (function e) fuel, eval (argument e) fuel with
            | some function, some argument => eval (body function argument) fuel
            | _, _ => none
  eval
