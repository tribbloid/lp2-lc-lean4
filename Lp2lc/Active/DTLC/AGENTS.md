...
- Strong HOAS (higher order abstract syntx) + F-Bound: it's almost identical to raw Strong HOAS (see @Tests/STLC_syntax.scala for a Scala example), with one minor twist: a binded variable in a function is no longer a pending `Val`, but an unknown `{I : Index}` that can correspond to syntax or runtime values through an `FBound` and `FBound.Aux`. Any concrete syntax rule or AST definition can only depend on the given `{I : Index}` and the required F-Bound environment.
