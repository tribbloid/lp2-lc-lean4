...
-- [Coq L24-29, L32-37] Types and terms
mutual
...
-- [Coq L68-81, L84-97] Local closure (mutual)
mutual
...
/-- [Coq L94, L132-145] binds -/
@[simp] def binds (x : Var) (U : typ) (E : env) : Prop := E.lookup x = some U

-- [Coq L116-145] Well-formedness (mutual)
mutual
  inductive wft : env -> typ -> Prop where
    ...
    | wft_mem : (E : env) -> (b : Bool) -> (T1 : typ) -> wft E T1 -> wft E (typ_mem b T1)
...
-- [Coq L160-205] Subtyping & has (mutual)
mutual
  inductive sub : env -> typ -> typ -> Prop where
    ...
    | sub_sel1 : (E : env) -> (U : typ) -> (t : trm) -> has E t (typ_mem false U) -> sub E (typ_sel t) U
    | sub_sel2 : (E : env) -> (S : typ) -> (t : trm) -> has E t (typ_mem true S) -> sub E S (typ_sel t)
    | sub_mem_false : (E : env) -> (b1 : Bool) -> (T1 T2 : typ) -> sub E T1 T2 -> sub E (typ_mem b1 T1) (typ_mem false T2)
    | sub_mem_true  : (E : env) -> (T1 T2 : typ) -> sub E T1 T2 -> sub E T2 T1 -> sub E (typ_mem true T1) (typ_mem true T2)
...
-- [Coq L298-315] substitution (mutual)
mutual
...
/-- [Coq L1423-L1451] psub: declarative subtyping at empty env for inversion -/
inductive psub : typ -> typ -> Prop where
  ...
  | psub_mem_false : (b1 : Bool) -> (T1 T2 : typ) -> psub T1 T2 -> psub (typ_mem b1 T1) (typ_mem false T2)
  | psub_mem_true  : (T1 T2 : typ) -> psub T1 T2 -> psub T2 T1 -> psub (typ_mem true T1) (typ_mem true T2)
  | psub_all : (L : Vars) -> (S1 S2 T1 T2 : typ) ->
      psub T1 S1 -> (∀ (x : Var), x ∉ L -> sub [(x, T1)] (S2 open_t_var x) (T2 open_t_var x)) -> psub (typ_all S1 S2) (typ_all T1 T2)
  | psub_trans : (S T U : typ) -> psub S T -> psub T U -> psub S U

/-- [Coq L1478-L1493] possible_types relation -/
inductive possible_types : Nat -> trm -> typ -> Prop where
  | pt_top : (n : Nat) -> (v : trm) -> value v -> wfe [] v -> possible_types n v typ_top
  | pt_mem_true : (n : Nat) -> (T T' : typ) -> psub T T' -> psub T' T -> possible_types n (trm_mem T) (typ_mem true T')
  | pt_mem_false : (n : Nat) -> (T U : typ) -> psub T U -> possible_types n (trm_mem T) (typ_mem false U)
  | pt_all : (L : Vars) -> (n : Nat) -> (V V' : typ) -> (e1 : trm) -> (T1 T1' : typ) ->
      (∀ (X : Var), X ∉ L -> typing [(X, V)] (e1 open_e_var X) (T1 open_t_var X)) ->
      psub V' V -> (∀ (X : Var), X ∉ L -> sub [(X, V')] (T1 open_t_var X) (T1' open_t_var X)) ->
      possible_types (n + 1) (trm_abs V e1) (typ_all V' T1')
  | pt_all_shallow : (V V' : typ) -> (e1 : trm) -> (T1' : typ) ->
      wfe [] (trm_abs V e1) -> wft [] (typ_all V' T1') ->
