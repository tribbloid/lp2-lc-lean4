            ...
            | yield argResult =>
              ...
              | some fnValue =>
                ...
                | fn body tIn =>
                  ...
                  | some input =>
                    cases hBody :
                        (body (env.valueRefs.save (AST.Trm.val (.fn body tIn), ()) { val := input, property := env.canEvalAny input })).eval fuel with
                    | outOfFuel =>
                      simp [AST.Trm.eval, hFn, hArg, hBody] at hEval
                    | yield bodyResult =>
                      have hBodyTop :=
                        ih fuel (Nat.lt_succ_self fuel)
                          (body (env.valueRefs.save (AST.Trm.val (.fn body tIn), ()) { val := input, property := env.canEvalAny input }))
