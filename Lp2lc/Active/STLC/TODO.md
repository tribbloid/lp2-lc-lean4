...
- [x] Reclassify `Trm.applyidFnOnItself` and `Trm.idFnOnFalse2` as malformed typing examples. `idFn` is annotated as `.fn .primitive .primitive`, so passing `idFn` itself as its primitive argument is not typable in STLC even though the untyped evaluator can reduce it.
