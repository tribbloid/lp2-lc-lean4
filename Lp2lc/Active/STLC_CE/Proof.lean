...
theorem termEvalMonotone {ctx : AST.Ctx}
  ...
  | ind fromFuel ih =>
    cases fromFuel with
    | zero =>
      cases trm <;> simp [Trm.eval] at hEval
    | succ fuel =>
      cases more with
      | zero => cases hFuel
      | succ toFuel =>
        ...
        cases trm with
        | val value =>
          cases value <;> simpa [Trm.eval] using hEval
        | apply fnTerm arg =>
          cases hFn : Trm.eval fnTerm env fuel with
          | outOfFuel => simp [Trm.eval, hFn] at hEval
          | yield fnResult =>
            have hFnTop := ih fuel (Nat.lt_succ_self fuel) env fnTerm toFuel fnResult hFuelTail hFn
            cases hArg : Trm.eval arg env fuel with
            | outOfFuel => simp [Trm.eval, hFn, hArg] at hEval
            | yield argResult =>
              have hArgTop := ih fuel (Nat.lt_succ_self fuel) env arg toFuel argResult hFuelTail hArg
              cases fnResult with
              | none =>
                simpa [Trm.eval, hFn, hArg, hFnTop, hArgTop] using hEval
              | some fnValue =>
                ...
                | fn tIn body =>
                  ...
                  | some input =>
                    ...
                    cases hBody : Trm.eval (body .ptop) (savedEnv.snoc inputEnv inputValue) fuel with
                    | outOfFuel =>
                      simp [Trm.eval, hFn, hArg, hBody] at hEval
                    | yield bodyResult =>
                      have hBodyTop :=
                        ih fuel (Nat.lt_succ_self fuel)
                          (savedEnv.snoc inputEnv inputValue) (body .ptop) toFuel bodyResult
                          hFuelTail hBody
                      simpa [Trm.eval, hFn, hArg, hFnTop, hArgTop, hBody, hBodyTop] using hEval
        | ref top =>
          simpa [Trm.eval] using hEval
