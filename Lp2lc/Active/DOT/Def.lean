...
| selfBinder (body: (this: I) -> Typ) : Typ -- AKA Mu-type, body can refer to `this` (If de Bruijn serial is used instead of PHOAS, `this` will have serial "0")
...
| top  : Typ -- `Any`
| bottom  : Typ -- `Nothing`
-- below are for generics, not part of DOT
...
| object (body : (this: I) -> ObjectBody Entry) : Val -- object/record with a member lookup that can refer to `this`, DOT only uses structural typing so Trait has to carry an extra hidden type member
