...
These mirror Coq's `subst_t_open_t` and `subst_e_open_e`. We keep them as
placeholders for now; they will be discharged later in the proof development.
-/
@[simp] theorem subst_t_open_t
  (T : typ) (t2 : trm) (z : Var) (u : trm) (hu : def_term u) :
  subst_t z u (open_t T t2) = open_t (subst_t z u T) (subst_e z u t2) := by
  -- TODO: prove by mutual induction on T/e; needed for binder cases
  sorry

@[simp] theorem subst_e_open_e
  (t1 : trm) (t2 : trm) (z : Var) (u : trm) (hu : def_term u) :
  subst_e z u (open_e t1 t2) = open_e (subst_e z u t1) (subst_e z u t2) := by
  -- TODO: prove by mutual induction on t1
  sorry
