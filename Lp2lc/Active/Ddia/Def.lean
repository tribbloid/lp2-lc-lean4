...
-- [Coq L75-99, 102-115] Local closure (mutual)
mutual
  inductive def_type : typ -> Prop where
    | type_bot : def_type typ_bot
    | type_top : def_type typ_top
    | type_and : (T1 T2 : typ) -> def_type T1 -> def_type T2 -> def_type (typ_and T1 T2)
    | type_or  : (T1 T2 : typ) -> def_type T1 -> def_type T2 -> def_type (typ_or T1 T2)
...
/-- [Coq L127] Environment as list of bindings -/
...
/-- [Coq L88-89,121-129] binds -/
@[simp] def binds (x : Var) (U : typ) (E : env) : Prop := E.lookup x = some U

-- [Coq L134-175] Well-formedness (mutual)
mutual
  inductive wft : env -> typ -> Prop where
    | wft_bot : (E : env) -> wft E typ_bot
    | wft_top : (E : env) -> wft E typ_top
    | wft_and : (E : env) -> (T1 T2 : typ) -> wft E T1 -> wft E T2 -> wft E (typ_and T1 T2)
    | wft_or  : (E : env) -> (T1 T2 : typ) -> wft E T1 -> wft E T2 -> wft E (typ_or T1 T2)
...
mutual
  inductive sub : env -> typ -> typ -> Prop where
    ...
    | sub_and11 : (E : env) -> (T1 T2 T : typ) -> wft E T2 -> sub E T1 T -> sub E (typ_and T1 T2) T
    | sub_and12 : (E : env) -> (T1 T2 T : typ) -> wft E T1 -> sub E T2 T -> sub E (typ_and T1 T2) T
    | sub_and2  : (E : env) -> (T T1 T2 : typ) -> sub E T T1 -> sub E T T2 -> sub E T (typ_and T1 T2)
    | sub_or21  : (E : env) -> (T T1 T2 : typ) -> wft E T2 -> sub E T T1 -> sub E T (typ_or T1 T2)
    | sub_or22  : (E : env) -> (T T1 T2 : typ) -> wft E T1 -> sub E T T2 -> sub E T (typ_or T1 T2)
    | sub_or1   : (E : env) -> (T1 T2 T : typ) -> sub E T1 T -> sub E T2 T -> sub E (typ_or T1 T2) T
    ...
    | sub_all : (L : Vars) -> (E : env) -> (S1 S2 T1 T2 : typ) ->
        sub E T1 S1 -> (∀ (x : Var), x ∉ L -> sub ((x, T1) :: E) (S2 open_t_var x) (T2 open_t_var x)) ->
  ...
  inductive has : env -> trm -> typ -> Prop where
    ...
    | has_abs : (E : env) -> (V : typ) -> (e : trm) -> (T : typ) ->
        okt E -> wfe E (trm_abs V e) -> wft E (typ_all V T) -> has E (trm_abs V e) (typ_all V T)
...
-- [Coq L355-375] substitution (mutual)
mutual
...
/-- [Coq L1491-1543] Pseudo-subtyping on empty env -/
inductive psub : typ -> typ -> Prop where
  | psub_bot : (U : typ) -> wft [] U -> psub typ_bot U
  | psub_top : (S : typ) -> wft [] S -> psub S typ_top
  | psub_and11 : (T1 T2 T : typ) -> wft [] T2 -> psub T1 T -> psub (typ_and T1 T2) T
  | psub_and12 : (T1 T2 T : typ) -> wft [] T1 -> psub T2 T -> psub (typ_and T1 T2) T
  | psub_and2  : (T T1 T2 : typ) -> psub T T1 -> psub T T2 -> psub T (typ_and T1 T2)
  | psub_or21  : (T T1 T2 : typ) -> wft [] T2 -> psub T T1 -> psub T (typ_or T1 T2)
  | psub_or22  : (T T1 T2 : typ) -> wft [] T1 -> psub T T2 -> psub T (typ_or T1 T2)
  | psub_or1   : (T1 T2 T : typ) -> psub T1 T -> psub T2 T -> psub (typ_or T1 T2) T
  ...
  | psub_all : (L : Vars) -> (S1 S2 T1 T2 : typ) ->
      psub T1 S1 -> (∀ (x : Var), x ∉ L -> sub [(x, T1)] (S2 open_t_var x) (T2 open_t_var x)) ->
      psub (typ_all S1 S2) (typ_all T1 T2)
  | psub_trans : (S T U : typ) -> psub S T -> psub T U -> psub S U

/-- [Coq L1575-1601] Possible types for values -/
inductive possible_types : Nat -> trm -> typ -> Prop where
  ...
  | pt_all : (L : Vars) -> (n : Nat) -> (V V' : typ) -> (e1 : trm) -> (T1 T1' : typ) ->
      ...
      psub V' V -> (∀ X, X ∉ L -> sub [(X, V')] (T1 open_t_var X) (T1' open_t_var X)) ->
  ...
  | pt_and : (n : Nat) -> (v : trm) -> (T1 T2 : typ) -> possible_types n v T1 -> possible_types n v T2 -> possible_types n v (typ_and T1 T2)
  | pt_or1 : (n : Nat) -> (v : trm) -> (T1 T2 : typ) -> possible_types n v T1 -> wft [] T2 -> possible_types n v (typ_or T1 T2)
  | pt_or2 : (n : Nat) -> (v : trm) -> (T1 T2 : typ) -> possible_types n v T2 -> wft [] T1 -> possible_types n v (typ_or T1 T2)
