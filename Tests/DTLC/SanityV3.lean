...
unsafe structure Handle where
  self: Val Handle

unsafe def ValAST := Val Handle
unsafe def TrmAST := Trm Handle

/-- generate a new handle if `self` is new, otherwise return the old handle -/
unsafe def ValAST.getHandle (self: ValAST) : Handle := .mk self

/-- return some if it's handle has been generated before, otherwise return none -/
unsafe def Handle.getTrm (self: Handle) : Option ValAST := sorry
