namespace ContextualEmbedding.SplitVal.CE
...
mutual
  inductive STLCCtx : Ctx -> Ty -> Type where
    | CVal : ValCtx ts t -> STLCCtx ts t
    | CVar [ReifyIndex ts ts'] : ProxyTop ts t -> STLCCtx ts' t
    | CApp : STLCCtx ts (a :-> b) -> STLCCtx ts a -> STLCCtx ts b

  inductive ValCtx : Ctx -> Ty -> Type where
    | CStar : ValCtx ts Unit
    | CLam : (ProxyTop (ts :/: a) a -> STLCCtx (ts :/: a) b) -> ValCtx ts (a :-> b)
end
open STLCCtx ValCtx
...
mutual
  ...
  def showValCtx : ValCtx ts t -> String
    | CStar => "CStar"
...
end ContextualEmbedding.SplitVal.CE
