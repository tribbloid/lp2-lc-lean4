...
Remark: this example is based on an example in the book [Certified Programming with Dependent Types](http://adam.chlipala.net/cpdt/) by Adam Chlipala, and is replicated from [Lean 4's `doc/examples/tc.lean`](https://github.com/leanprover/lean4/blob/master/doc/examples/tc.lean).
-/
inductive Expr where
  | nat : (v : Nat) → Expr
  | plus : (a : Expr) → (b : Expr) → Expr
  | bool : (v : Bool) → Expr
  | and : (a : Expr) → (b : Expr) → Expr
...
instance (e : Expr) (t : Ty) : Decidable (HasType e t) :=
  ...
  | .found t' ht' =>
    if heq : t = t' then
      ...
      isFalse λ ht => heq (HasType.det ht ht')
